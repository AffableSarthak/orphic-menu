...
const CartItem = ({ item, qty, idx, username }: { item: Iitem; qty: number; idx: number; username: string }) => {
      ...
      <div id="cartContainer" className="flex justify-center items-center gap-2 py-1">
        ...
        <div id="cartInfo" className="flex-seven flex flex-col justify-between">
          <div className="flex flex-row justify-between mb-2">
            ...
            {/*cart product name*/}
            <p className="text-sm text-smokyBlack font-medium">₹{(getCustPrice(item) + parseInt(item.price)) * qty}</p>
            {/*cart product price*/}
          </div>

          <div className="flex flex-row justify-between items-end">
            <p className="text-xs text-gray-400 font-medium">
              Added by <span className="text-secondary capitalize">{username}</span> {/* user name*/}
            </p>
