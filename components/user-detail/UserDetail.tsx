...
const UserDetail = (props) => {
  const { isLoading, setUsername, getCategories, populateGc } = useContext(sessionContext);
  // console.log(props)
      ...
      <div className="mt-5">
        <Formik
          ...
          validationSchema={yup.object({
            username: yup.string().required("Username is Required!").min(3, "Atleast enter two characters").max(15),
          })}
          onSubmit={async (values, formikHelpers) => {
            await setUsername(values.username);
            if (localStorage.getItem("username").length > 2 || localStorage.getItem("rId") !== undefined) {
              router.push({
